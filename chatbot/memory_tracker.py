# memory_tracker.py - Clean version without unicode issues
import json
import time
...
class HybridMemoryTracker:
    """
    Hybrid memory system that works with both RASA and RAG:
    - RASA slots for structured conversation state
    ...
    def __init__(self, memory_db_path="memory.db", max_history=20):
        ...
        print("Hybrid Memory Tracker initialized")
    ...
    def init_memory_db(self):
        ...
        try:
            ...
            # Current session state table (RASA slots backup)
            cursor.execute('''
    ...
    def update_user_interests(self, user_id: str, message: str, entities: List[Dict] = None):
        ...
        # Track mentioned artists
        artists = ["van gogh", "delaroche", "traquair", "picasso", "monet", "da vinci"]
        for artist in artists:
            if artist in message_lower:
                conversation["mentioned_artists"].add(artist)
        
        # Track topics of interest
        art_topics = {
            "colors": ["color", "colours", "blue", "red", "yellow", "bright"],
            "techniques": ["technique", "brush", "paint", "style", "method"],
            "history": ["history", "period", "time", "when", "era"],
            "stories": ["story", "meaning", "about", "tale", "narrative"]
        }
        
        for topic, keywords in art_topics.items():
        ...
        if entities:
            for entity in entities:
                if entity.get("entity") == "artwork":
                    conversation["current_topic"] = entity.get("value")
                elif entity.get("entity") == "artist":
                    conversation["current_topic"] = f"artist_{entity.get('value')}"
    ...
    def get_conversation_context(self, user_id: str, last_n: int = 3) -> Dict[str, Any]:
        ...
        context = {
            ...
            "mentioned_artists": list(conversation.get("mentioned_artists", [])),
    ...
    def get_personalized_context(self, user_id: str) -> str:
        """Generate personalized context string for RAG/DialoGPT"""
        context = self.get_conversation_context(user_id)
        
        if not context:
            return "This is a new visitor to the museum."
        
        context_parts = []
        
        # Add conversation context
        if context.get("mentioned_artists"):
            artists = ", ".join(context["mentioned_artists"])
            context_parts.append(f"User has shown interest in: {artists}")
    ...
    def update_rasa_slots(self, user_id: str, slots: Dict[str, Any]):
        """Update RASA slots and store as backup"""
        try:
            conn = sqlite3.connect(self.memory_db_path)
            cursor = conn.cursor()
            ...
            enhanced_slots.update({
                "mentioned_artists": context.get("mentioned_artists", []),
...
def test_memory_system():
    ...
    memory = HybridMemoryTracker(memory_db_path="test_memory.db")
    
    # Simulate conversation
    user_id = "test_user"
    
    conversations = [
        ("Hello, I'm interested in Van Gogh", "Hello! Van Gogh is amazing! What would you like to know?", "greet"),
        ("What style did he use?", "Van Gogh used thick brushstrokes called impasto...", "ask_style"),
        ("Tell me about colors", "Van Gogh loved bright yellows and deep blues...", "ask_colors"),
        ("Compare him to Picasso", "Van Gogh and Picasso both revolutionized art...", "compare_artists")
    ]
    
    print("Simulating conversation...")
    for i, (message, response, intent) in enumerate(conversations):
        print(f"  {i+1}. User: {message[:30]}...")
        ...
        print(f"     Context: {context.get('current_topic', 'None')} | Interests: {len(context.get('interests', []))}")
    
    # Test memory retrieval
    print("\nMemory Summary:")
    summary = memory.get_memory_summary(user_id)
    print(f"  Total interactions: {summary.get('total_interactions')}")
    print(f"  Current interests: {summary.get('current_session', {}).get('interests', [])}")
    print(f"  Mentioned artists: {summary.get('current_session', {}).get('mentioned_artists', [])}")
    
    # Test personalized context
    personalized = memory.get_personalized_context(user_id)
